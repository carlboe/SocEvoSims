...
msg0 <-paste(
"Baseline (50% sg/ 0% mat/ 50% kin5) with modification so density is based on consumption weighted population.",
"3 populations of approx 3333 each, loosly coupled through total density dependence, but otherwise separate",
"Popsize:10000, begin with each indiv. her own matriarchy with a unique ancestry; ",
"arbitrary groupings of 8 for social groups. Dispersion:None, Fusion:TMfusion, ",
"Fission:TMfission/kin5 together. ,  Gamma.kin uses kin5",   
"New mortality model: hazard depends on ( K(x)*delta + background.mort) *gamma^(e.mortality)",
" Lethality",
"of 0.02\n Kaplan-derived production and consumption schedules ",
  sep="\n");
...
gamma <- timapply2(test$sharing.gamma,test$ages,mean); # mean w/in age class
t.gamma <- gamma.series[t1:t2,];     #mean age profile of sharing gamma for each time
